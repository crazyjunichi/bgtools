...
src/
  ...
  shared/
    ...
    players/           # 全局玩家名单 —— 跨工具共享的一份数据 + 16 色板
...
- **计分纸**：固定条目逐项结算的矩阵（横向是人、纵向是条目），点格子用右侧键盘输入；条目可设「每个 N 分」，格子里填数量、得分自动折算；自带游戏模板（通用空白 / 农场主），切模板不清分数，只是把用不到的条目收起来
...
- **随机指针**：转一下指个方向，报大致钟点
- **玩家名单**：桌上是谁、叫什么、拿哪色棋子。**这份名单跨工具共享**，各工具的「换人」直接从这里选；16 色板刻意避开语义色，颜色允许重复但一定同时出名字
- **设置**：中英切换
