...
- 颜色只用 [colors.ts](src/shared/players/colors.ts) 的显式映射表：用户卡片一律 `PLAYER_LINE`（无底色 + 底部粗色边，线宽由调用点按尺寸档补 `border-b-*`）；`PLAYER_SOLID` 只剩选色板与 touch-pick 圆环这类「展示颜色本身」的用途；`PLAYER_SOFT` / `PLAYER_DOT` 给只要带身份色的列表行。15 色刻意避开 rose / emerald / sky / amber 四个语义色，末尾三格是中性/大地色（棕 / 灰 / 墨；棕在 `@theme` 自定义，墨是主题相反色：深主题呈亮、浅主题呈深，canvas 侧走 `playerHexOf` 按纸面取反）。旧存档的白/黑以别名留在表尾渲染，新代码不许再产生这两个值。**这已是上限，不要再加色**，依据见 [docs/DESIGN.md](docs/DESIGN.md) §2。**同色允许被两个玩家共用**（超过 15 人必然重复），所以任何露出玩家色的地方必须同时出名字或色名 —— 颜色不许是唯一识别编码。文字色由这几张表给，**调用点不许在 `PLAYER_SOLID` 后面再补 `text-*`**（会覆盖掉个别档自带的文字色）
...
生成、站内扫码、外部进入三层共用一条约定：**码的内容就是完整本站 URL（`#/…`），hash 路由即协议** —— 「这个码能干什么」由路由表回答，各环节不许认识彼此的业务参数。
...
**唯一例外是战绩统计（stats）**：它不上桌，是一个人近距翻看历史的阅读页，所以允许页面级滚动、不套 ToolLayout、列表行降到 44px（依据同 `short` 档：手持近距）。这个例外只对 stats 成立，别的工具不许引用它放行自己。
...
- **返回/通用小工具入口由 [AppHeader](src/AppHeader.tsx) 统一提供**，工具页里不要自己画返回键、标题栏，也不要自己实现全屏。它**常显且正常占位**：竖屏通栏、横屏变左侧 64px 竖条（**不要改回自动收起的 overlay**，理由见 [docs/DESIGN.md](docs/DESIGN.md) §8）。全屏不再是独立按钮 —— `screen.orientation.lock()` 只在全屏或已安装 PWA 下生效，全屏因此降级成朝向切换内部的前提步骤；朝向切换点按频率低，收在设置面板里（[useOrientation](src/shared/hooks/useOrientation.ts)）
...
理由不是「注释要短」，是这类内容**必然失同步**：把 `bg-ink/40` 调成 `/50` 时没人会回头重算注释里那个明度值，于是注释开始说谎，比没有更糟。同一个数字在文件头和逐条注释里各写一遍更是必漏 —— 拆弹三态的明度表和装备卡的透明度都已经因此烂过一次。
...
- **保留**这三类：领域事实（12 根导线、5 张装备、15 色上限）、CSS 写法本身（`height: 100%` 而不是 `100dvh`）、被否决的旧方案引用
