...
export function bgStatsUrl(match: MatchDraft & { legacy?: boolean }): string | null {
  ...
  // 目录游戏按 bggId 精确匹配；目录外的用手填的名字，对方靠名字匹配
  if (!game && !match.gameName) return null
  const t = en()
  ...
  const data = {
    ...
    comments: match.note,
    game: {
      name: game ? t(game.nameKey) : match.gameName,
    ...
    players: match.players.map((p) => {
      ...
      return {
        ...
        // 规格里这个键未标注 optional（示例恒出现），缺了会被拒；我们不记先手，恒 false
        startPlayer: false,
