/**
 * 落盘与分享。留在工具目录而不上提 `shared/`：目前只有计分纸一个出口
 * （「第二处用到才上提」）。
 */
...
export function canShareBlob(blob: Blob, filename: string): boolean {
  return typeof navigator.canShare === 'function' && navigator.canShare({ files: [toFile(blob, filename)] })
}
...
export async function shareBlob(blob: Blob, filename: string, title: string) {
  try {
    ...
    if ((e as Error)?.name !== 'AbortError') console.warn('[score-sheet] share failed', e)
  }
}
