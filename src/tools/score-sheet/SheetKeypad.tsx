...
type Props = {
  ...
  onNext: () => void
  /**
   * 未选中格子时竖屏只留底部按钮行，矩阵回到一屏全览；
   * 横屏不缺这点高度，键盘常驻右栏（disabled 置灰即「还没选格子」的表达）
   */
  collapsed?: boolean
  /** 从游戏卡进入的固定表没有换模板这一步，整个模板按钮不出现 */
...
export function SheetKeypad({
  ...
  return (
    <div className="flex shrink-0 flex-col gap-2 wide:w-64">
       ...
       */}
      <div
        className={`shrink-0 flex-col gap-1 rounded-xl bg-surface-2 p-2 short:gap-0.5 short:p-1.5 ${
          collapsed ? 'hidden wide:flex' : 'flex'
        }`}
      >
        ...
        <div className="flex min-h-7 items-center short:min-h-6">
          {active && seat && (
            <span
              className={`w-full truncate rounded-md border-b-2 px-2 py-1 text-center text-sm font-bold short:py-0 ${
         ...
         */}
        <div className="flex min-h-5 items-baseline justify-center gap-1.5 short:min-h-4">
          <span
      ...
      <div
        className={`flex-1 grid-cols-4 grid-rows-4 gap-2 short:gap-1.5 ${
          collapsed ? 'hidden wide:grid' : 'grid'
        }`}
      >
