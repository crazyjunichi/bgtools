 ...
 *
 * 12 席之前行数是钉死的（一行或两行），卡高都在 386px 以上，装什么都够。13 席起才开始
 * 按 5 列排多行 —— 此时卡高掉到 ≈254px、16 席起更是只有 ≈195px，宽高同时吃紧。
 * 撤的是历史那三行而不是别的：它信息密度最低，而完整矩阵在「记录」浮层里随时能看。
 * 阈值就取 5 列开始的那一刻，而不是等到 16 席才撤 —— 人数只增不减，少跳一次布局。
 */
const DENSE_FROM = 13
 ...
 *
 * **字号跟着列数走，不跟人数走。** 卡宽 = 可用宽 / 列数，等宽数字高宽联动，四位合计要放进
 * 卡里，字号上限就是卡内容宽的约 1/2.4。横屏 1088px 下：4 列内容 ≈242px 放得下 74px 的字，
 * 6 列只剩 ≈150px 就必须降到 56px。所以 6 席（一行 6 列）的数字比 7 席（两行 4 列）的小 ——
 * 加一个人反而变大，是几何结果不是 bug。
 *
 * 竖屏列数一律比横屏少：可用宽只有横屏的约 2/3，高度反而多一半。
 */
 ...
 * 主界面：一人一张卡。
 *
 * **为什么不是矩阵**：矩阵把每人的宽度压到一列（≈96px），而合计那个数字是全桌整晚
 * 瞄得最多的东西，等宽字体高宽联动，列宽就是它的天花板。卡片把宽度还给人，合计能做到
 * 74px（矩阵里只有 36px）。代价是主界面没有"同一轮所有人排成一行"的横向对比 ——
 * 那个换到「记录」浮层（[ScoreHistory]）里，需要时才看，它本来也不是记分时看的东西。
 *
 * **整张卡是一个按钮**，点哪儿都开调分浮层（[SeatSheet]）：操作点和反馈点必须重合。
 *
 * 行高 `minmax(8.5rem,1fr)`：人少时 `1fr` 平分容器高度把卡拉满，多到连密集档都装不下时
 * 锁住 136px 改为框内纵滚 —— 席位数不设上限，总有排不完的时候，但那时页面仍不滚。
 */
      ...
      {seats.map((s, i) => {
        ...
        return (
          ...
          >
            ...
            {/*
             * 最近三轮：只回顾，不是主体，所以压到 text-xs 并靠分隔线降权。
             * `short:hidden` 不是嫌它挤 —— 844×390 下 6 席的卡内容高只有 156px，
             * 这三行占 53px，留给合计的就不够它的下限 40px 了。矮屏是手持场景，
             * 回顾本来就该去「记录」浮层。
             */}
