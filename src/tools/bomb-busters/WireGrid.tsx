 ...
 *
 * 亮度设计（明度按半透明色合成在 sky-950/40 区块底上估算）：
 * 亮格 sky-700 = 87 · 未拆暗格 sky-950 = 43 · 半拆暗格 ink/40 = 18 · 全拆整格 ink/70 = 14
 *
 * 底色是每格的**暗底**，亮的部分由下面 BAND_FILL 铺的不透明 sky-700：
 * 亮格必须不透明，否则叠在不同暗底上合成结果不同，未拆与半拆的亮格就对不上了；
 * 暗底反过来**故意**分三档，它是"拆到哪了"的第二层编码。
 */
const TONE: Record<DefuseState, string> = {
  0: 'border-sky-300 bg-sky-950 text-sky-50',
  // 半拆的暗底压到贴近全拆（18 vs 14）：一格里只剩 2/4 亮，暗底跟未拆一样亮时
  // 整格的平均亮度仍接近未拆，桌上斜视要靠数格子才分得出。压暗后半拆自己就往退场方向靠，
  // 与全拆的区分交给亮格数（2/4 vs 0/4）+ 实线边框 + 无删除线
  1: 'border-sky-500/70 bg-ink/40 text-sky-50',
