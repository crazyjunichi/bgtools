 ...
 *
 * 这里给的是每格的**暗底**，亮的部分由下面 BAND_FILL 铺。两条不能破的：
...
const TONE: Record<DefuseState, string> = {
  ...
  // 半拆的暗底压到贴近全拆：一格里只剩 2/4 亮，暗底跟未拆一样亮时整格的平均亮度仍接近未拆，
  // 桌上斜视要靠数格子才分得出。压暗后半拆自己就往退场方向靠，
  // 与全拆的区分交给亮格数（2/4 vs 0/4）+ 实线边框 + 无删除线
  1: 'border-sky-500/70 bg-ink/40 text-sky-50',
  // 全拆退场：拆完的号码不用再读，压到最暗让还没拆的自己浮出来。
  // 虚线边框保住"这格已经不是活的"这层非颜色编码
