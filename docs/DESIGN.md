...
计分纸的导出图有两套配色（[png/paint.ts](../src/tools/score-sheet/png/paint.ts) 的 `DARK` / `PRINT`，用户在预览里用左右箭头切）。

**这不违反「深色单一主题」**：那条约束管的是**应用界面**——同一块屏、同一个视距、同一晚，换主题只会让人重新认一遍。导出图反过来，它的全部用途就是**脱离应用**：发进别人的聊天窗（浅色居多）、贴进文档、打印出来夹在规则书里。深底图打印费墨，浅底纸上的深色表格才是那张纸本来的样子。

| 角色 | `DARK` | `PRINT` |
|---|---|---|
| 底 / 格子 / 分隔线 | 照抄 `surface` / `surface-2` / `line` | 白 / 极浅灰 / `zinc-400` |
| 正文 / 次要 / 最暗 | 照抄 `text` / `text-muted` / `text-dim` | `zinc-900` / `zinc-700` / `zinc-500` |
| 正分 / 负分 / 领先 | 青绿 / 橙 / 琥珀，取 300 档的明度 | 同色相的 `teal` / `orange` / `yellow` **700 档** |

浅底那套有两处不是把深底那套反过来就能得到的：

- **语义色必须换到 700 档**。300 档是照着深底选的，放到白底上对比只有 2:1 上下，正负分的颜色区分当场消失。700 档对白底 ≥ 4.5:1，同时三个色相之间仍分得开
- **玩家胶囊必须描边**（`SheetPalette.chipStroke`，一个中性深灰）。16 色里的白与浅灰在白底上没有边界，胶囊会散成一片；这和深底上 `PLAYER_HEX.black.ring` 要解决的是同一个问题，只是方向相反。玩家色本身两套共用一张 hex 表（canvas 的 `fillStyle` 不认 `oklch()`，**静默失败**，所以只能走 `PLAYER_HEX`）
