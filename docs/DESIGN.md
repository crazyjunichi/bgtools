...
| 例子 | 编码 1 | 编码 2 |
|---|---|---|
| 拆弹三态 | sky 单色亮度阶梯，**不换色相**（三态是同一件事的进度，色相跳变会读成三类不同的东西）。每格纵向 **4 等分 + 分割线**，规则是「**亮 = 还没拆的部分**」，从底往上填：未拆 4/4 · 半拆 2/4 · 全拆 0/4。亮格 `sky-700`（Y87）· 暗格 `sky-950`（Y41）· 全拆整格 `bg-ink/70`（Y21，比半拆的暗格还暗一档）。两处颜色**必须都是不透明色**（半透明叠在不同底上合成结果不同，未拆与半拆的亮格就对不上）；4 等分不对应任何局内数量，作用是把"拆了多少"变成可数的格数，斜视下数格子比比亮度可靠 | `½` / `✓` 角标 + 填充格数 + 删除线 + 全拆改虚线边框（**刻意保留字形**：这俩是压在内容格上的排版记号，跟数字同处一个字号体系，换成 SVG 会与编号错位）。图例块 12px 塞不进分割线，只保留填充量满 / 半 / 空 |
