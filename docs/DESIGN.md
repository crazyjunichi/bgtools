...
- **底线档的「白」与「黑」互换深度**：白线（zinc-100）在浅主题白卡上隐形，黑线（zinc-950）在近黑底上隐形，所以白 = `zinc-100 light:zinc-400`、黑 = `zinc-400 light:zinc-950`

调用点因此**不许在 `PLAYER_SOLID` 之后再补 `text-*`**：那会把黑的白字覆盖回去。文字色一律由这几张表给（`PLAYER_LINE` 卡面不着色，文字保持默认色）。

**同色允许被两个玩家共用**（偏好比唯一性重要，调色板只提示"谁也在用"、不禁用）。**人数本身不设上限**，所以超过 16 人时重复是必然的 —— 颜色永远不是唯一识别编码，硬性要求：任何露出玩家色的地方必须同时出**名字**或**色名**，选中态再叠 `✓`（见 §6）。

### 导出图调色板

计分纸的导出图有两套配色（[png/paint.ts](../src/tools/score-sheet/png/paint.ts) 的 `PRINT` / `DARK`，用户在预览里用左右箭头切，**浅底是默认**）。
...
- **玩家色条必须描边**（`SheetPalette.swatchStroke`，一个中性深灰）。16 色里的白与浅灰在白底上没有边界，色条会直接消失；这和深底上 `PLAYER_HEX.black.ring` 要解决的是同一个问题，方向相反。玩家色本身两套共用一张 hex 表（canvas 的 `fillStyle` 不认 `oklch()`，**静默失败**，所以只能走 `PLAYER_HEX`）
