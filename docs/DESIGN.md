...
玩家名单面板（顶栏 👥）身份色用 `teal`：四个语义色都不能占，`amber` / `sky` / `violet` 已分给快速骰子 / 计时器 / 随机指针。**调色板扩到 15 色后 `teal` 也成了玩家色**，这个重叠无法回避（非语义色相一共只有 12 个，全给了玩家），改由位置区分：accent 只出现在操作条按钮上，玩家色只出现在名字旁 —— 新面板挑 accent 时别再指望"找一个没被玩家占的色相"。
...
[shared/players/colors.ts](../src/shared/players/colors.ts) 的 15 色，**刻意一个都不碰语义色**：玩家色要能和"危险 / 完成 / 信息 / 警告"同屏共存，撞色就分不出哪个是语义。取的都是桌上实物棋子常见的颜色，玩家能直接对上手里的棋子。

排除 rose / emerald / sky / amber 后，Tailwind 只剩 **12 个可用色相**，而格子要"互相不像"，所以后 3 格换维度取 —— 中性色与大地色跟任何色相都不可能看混。四行就是四个色域块：
...
| 中性与大地 | 棕 / 灰 / 墨 | `brown`（自定义）/ `zinc-400` / 主题相反色 |

**这已经是 15 色的上限，不要再加。** 剩下的色相全在四个语义色上，中性维度也只剩"更暗的灰"—— 深底上再往下就分不出来了。最近的两对是 松绿/青（相差 16°）与 靛/紫（19°），它们靠色名区分。

档位仍走上面那张表：`PLAYER_LINE` = `rounded-b-none border-b-<c>-400`（用户卡片的标准形态：卡面无底色、底部粗色边承载身份色；底角统一收方，粗边才是一条笔直的线而不顺圆角卷起；线宽由调用点按尺寸档补 `border-b-*`），`PLAYER_SOLID` = `bg-<c>-400 text-ink`（只剩选色板、touch-pick 圆环这类「展示颜色本身」的用途），`PLAYER_SOFT` = `border-<c>-500/60 bg-<c>-500/15 text-<c>-300`，`PLAYER_DOT` 只取 `bg-<c>-400`。四张都是显式 `Record`，禁止拼接。**soft 档已含 `bg-<c>-500/15`，不要再叠 `bg-surface-2`** —— 两条都是 `background-color`，谁赢取决于 CSS 生成顺序而非类名顺序。
...
- **「墨」不是一个固定色，是主题相反色**：深主题取亮、浅/墨水屏取深，四张类名表里都是成对的 `light:` 例外，文字色跟着翻。它顶替了旧的白/黑两格 —— 固定白在浅主题、固定黑在深主题都必然隐形；canvas 画板上旧方案给黑笔叠亮描边（`ring`），增量绘制的段间接缝会露描边、画成点状，已随这两格一起废弃
- canvas 场景拿不到主题类名：「墨」的 hex 由 `playerHexOf(color, tone)` 按纸面明暗现取（`paperTone()` 读 `<html data-theme>`，深色是默认无属性；组件侧用 [usePaperTone](../src/shared/hooks/usePaperTone.ts)），其余色查 `PLAYER_HEX` 表
- **旧存档里的「白/黑」**仍以别名留在各表尾（样式即当时那一档，黑的实心档仍带 `ring-2` 描边），色名回落到「墨」；新代码不许再产生这两个值

调用点因此**不许在 `PLAYER_SOLID` 之后再补 `text-*`**：那会覆盖掉个别档自带的文字色（墨、旧存档的黑）。文字色一律由这几张表给（`PLAYER_LINE` 卡面不着色，文字保持默认色）。

**同色允许被两个玩家共用**（偏好比唯一性重要，调色板只提示"谁也在用"、不禁用）。**人数本身不设上限**，所以超过 15 人时重复是必然的 —— 颜色永远不是唯一识别编码，硬性要求：任何露出玩家色的地方必须同时出**名字**或**色名**，选中态再叠 `✓`（见 §6）。

### 导出图调色板

计分纸的导出图有两套配色（[share/paint.ts](../src/shared/match/share/paint.ts) 的 `PRINT` / `DARK`，用户在预览里用左右箭头切，**浅底是默认**）。

**这不违反「深色单一主题」**：那条约束管的是**应用界面**——同一块屏、同一个视距、同一晚，换主题只会让人重新认一遍。导出图反过来，它的全部用途就是**脱离应用**：发进别人的聊天窗（浅色居多）、贴进文档、打印出来夹在规则书里。深底图打印费墨，浅底纸上的深色表格才是那张纸本来的样子。浅底因此排在前面 —— 分享与打印两个主流场景都偏它。

| 角色 | `PRINT` | `DARK` |
|---|---|---|
| 底 | 纯白 | 近黑 |
| 规则线 | `zinc-400` | 中性灰，能在近黑上立住 |
| 正文 / 次要 / 最暗 | `zinc-900` / `zinc-700` / `zinc-500` | 纯白 / 浅灰 / 中灰 |
| 负分 / 领先 | `orange` / `yellow` 的 **700 档** | 橙 / 金，比屏幕上那套**更饱和** |

四条硬约束：

- **两套都不照抄 `@theme`。** 深色那套曾经逐行等于 `surface` / `surface-2` / `line` / `text`，那是「久盯不刺眼」的夜间取值；导出图的目标是「聊天流缩略图里一眼读清、被二次压缩也不糊」，所以底压得更黑、正文提到纯白、语义色加饱和
- **没有「正分色」。** 屏幕上给正分上色是为了边填边确认符号；一张已结算的图上，颜色只留给**异常**（负分）与**结果**（合计、领先者）。明细行整片染色的实测后果是把合计那排压成了次要的，恰好反了 —— 层级靠字号和位置给，不靠色相
- **浅底的语义色必须换到 700 档**。300 档是照着深底选的，放到白底上对比只有 2:1 上下，负分与领先者的区分当场消失。700 档对白底 ≥ 4.5:1，两个色相之间仍分得开
- **玩家色条必须描边**（`SharePalette.swatchStroke`，一个中性深灰，浅底外观才需要）。「黄」这类亮色条在白底上没有边界，会直接消失。玩家色两套外观共用同一组 hex（canvas 的 `fillStyle` 不认 `oklch()`，**静默失败**，只能走 `PLAYER_HEX` 字面量表）；「墨」没有固定值，由 `playerHexOf` 按调色板自带的 `tone`（纸面明暗）取反

调色板是 form 渲染器的**入参**，不是它内部的常量：外观与内容形式正交，`skin × form` 的组合数不许进代码量。加一种外观 = 加一张表 + 注册表加一行，不用把每个 form 改一遍。
...
**上面整节的取值一条都不适用于导出图。** 那些数字全是从「平板平放桌上、多人斜视、视距 50–70cm、一屏放完不滚动」推出来的，而导出的 PNG 不在这个场景里：它被发进聊天窗、贴进文档、打印出来，读者拿在手上、可以点开放大、可以往下滚。所以 [png/paint.ts](../src/tools/score-sheet/png/paint.ts) 自带一套尺度，与 `@theme` 和触控规范都不共享数值。

被换掉的三条与理由：

| 屏幕上 | 导出图 | 理由 |
|---|---|---|
| 行高 = 触控目标 56px | 行高 = `lineH(字号)`，系数 1.6 | 图片没人点。照着 56 排出来的图整体偏高，发到群里被缩略得更狠，**字反而更小** |
| 最小字号 12px、正文 16px 起 | `TYPE` 阶梯，`label` 是正文基准 | 12px 下限是斜视距离的产物。图片的层级只需内部自洽，靠比例而不是绝对下限 |
| 长名字 `truncate` / `fit` 截断 | 折行（`wrap`），宽度由内容定 | 截断是「一屏放完」的代价，导出图没有这个约束。只有宽度真锁死的地方（列头）还用 `fit` |

版面本身是**无框**的（用户在三种风格里选的）：不画格子底、不画逐行分隔线，靠留白与右对齐的数位分列，只在标题下和合计上各压一条规则线。两条连带的硬约束：

- **规则线不许改细。** 微信/Telegram 会对图片再压一遍，1px 发丝线整条消失过。所以规则线 2px 起，玩家色条 4px 厚
- **无框 ⇒ 行距和列距不能压。** 没有框线兜着，间距一小几行数字就连成一团。所有间距都是留白单位 `U` 的倍数，改密度改 `U` 而不是逐处调
- **领先者是双编码**：best 色 + 数字下方一条短线。压缩后色差会失真，只靠颜色标冠军不可靠（同 §6 的规矩，那条对导出图同样有效）
...
| 玩家身份 | 玩家色底部粗边（卡面不着色） | **始终带名字**；调色板格子带中文色名，选中加 `IconCheck`（同色可共用，颜色靠不住） |
| 名单里选中的玩家 | 玩家色底部粗边 | `IconCheck` 角标；面板左侧列表的选中态干脆不用玩家色，走 `surface-3` + `IconSelected` |
